...
/**
 * KDAPHPExcel_Writer_Excel5_Parser
 *
 * @category   KDAPHPExcel
 * @package    KDAPHPExcel_Writer_Excel5
 * @copyright  Copyright (c) 2006 - 2013 KDAPHPExcel (http://www.codeplex.com/KDAPHPExcel)
 */
class KDAPHPExcel_Writer_Excel5_Parser
{
	/**	Constants				*/
	...
	function _convert($token)
	{
		...
		throw new KDAPHPExcel_Writer_Exception("Unknown token $token");
	}
	...
	function _convertNumber($num)
	{
		...
		if ((preg_match("/^\d+$/", $num)) and ($num <= 65535)) {
			...
			if (KDAPHPExcel_Writer_Excel5_BIFFwriter::getByteOrder()) { // if it's Big Endian
				$num = strrev($num);
			}
	...
	function _convertString($string)
	{
		...
		$string = cp_substr($string, 1, cp_strlen($string) - 2);
		if (cp_strlen($string) > 255) {
			throw new KDAPHPExcel_Writer_Exception("String is too long");
		}

		return pack('C', $this->ptg['ptgStr']) . KDAPHPExcel_Shared_String::UTF8toBIFF8UnicodeShort($string);
	}
...
//		$volatile = $this->_functions[$token][3];
	...
	function _convertRange2d($range, $class=0)
	{
		...
		if (preg_match('/^(\$)?([A-Ia-i]?[A-Za-z])(\$)?(\d+)\:(\$)?([A-Ia-i]?[A-Za-z])(\$)?(\d+)$/', $range)) {
			...
			// TODO: use real error codes
			throw new KDAPHPExcel_Writer_Exception("Unknown range separator");
		}
		...
		if ($class == 0) {
			...
			// TODO: use real error codes
			throw new KDAPHPExcel_Writer_Exception("Unknown class $class");
		}
...
//		}
	...
	function _packExtRef($ext_ref)
	{
		...
		if (preg_match("/:/", $ext_ref)) {
			...
			if ($sheet1 == -1) {
				throw new KDAPHPExcel_Writer_Exception("Unknown sheet name $sheet_name1 in formula");
			}
			$sheet2 = $this->_getSheetIndex($sheet_name2);
			if ($sheet2 == -1) {
				throw new KDAPHPExcel_Writer_Exception("Unknown sheet name $sheet_name2 in formula");
			}
			...
			if ($sheet1 == -1) {
				throw new KDAPHPExcel_Writer_Exception("Unknown sheet name $ext_ref in formula");
			}
			$sheet2 = $sheet1;
		}
	 ...
	 * KDAPHPExcel_Writer_Excel5_Workbook class.
	 *
	 * @access	private
	 ...
	 *
	 * @access public
	 * @see KDAPHPExcel_Writer_Excel5_Workbook::addWorksheet()
	...
	function _cellToPackedRowcol($cell)
	{
		...
		if ($col >= 256) {
			throw new KDAPHPExcel_Writer_Exception("Column in: $cell greater than 255");
		}
		if ($row >= 65536) {
			throw new KDAPHPExcel_Writer_Exception("Row in: $cell greater than 65536 ");
		}
	...
	function _rangeToPackedRange($range)
	{
		...
		if (($row1 >= 65536) or ($row2 >= 65536)) {
			throw new KDAPHPExcel_Writer_Exception("Row in: $range greater than 65536 ");
		}
	...
	function _cellToRowcol($cell)
	{
		...
		$expn   = cp_strlen($col_ref) - 1;
		$col    = 0;
		$col_ref_length = cp_strlen($col_ref);
	...
	function _advance()
	{
		...
		$formula_length = cp_strlen($this->_formula);
		// eat up white spaces
		if ($i < $formula_length) {
		...
		while ($i < $formula_length) {
			...
			if ($this->_match($token) != '') {
				//if ($i < cp_strlen($this->_formula) - 1) {
				//    $this->_lookahead = $this->_formula[$i+1];
				//}
	...
	function _match($token)
	{
		switch($token) {
			...
			default:
				...
				elseif (preg_match("/\"([^\"]|\"\"){0,255}\"/",$token) and $this->_lookahead != '"' and (kda_substr_count($token, '"')%2 == 0))
				{
					return $token;
				}
	...
	function _fact()
	{
		if ($this->_current_token == "(") {
			...
			if ($this->_current_token != ")") {
				throw new KDAPHPExcel_Writer_Exception("')' token expected.");
			}
			$this->_advance();         // eat the ")"
			return $result;
		}
		...
		throw new KDAPHPExcel_Writer_Exception("Syntax error: ".$this->_current_token.
	...
	function _func()
	{
		...
		/**/
			if ($num_args > 0) {
					...
					$this->_current_token == ";")
				{
					...
					throw new KDAPHPExcel_Writer_Exception("Syntax error: comma expected in ".
		...
		if (!isset($this->_functions[$function])) {
			throw new KDAPHPExcel_Writer_Exception("Function $function() doesn't exist");
		}
		$args = $this->_functions[$function][1];
		// If fixed number of args eg. TIME($i,$j,$k). Check that the number of args is valid.
		if (($args >= 0) and ($args != $num_args)) {
			throw new KDAPHPExcel_Writer_Exception("Incorrect number of arguments in function $function() ");
		}
