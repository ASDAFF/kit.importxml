...
/**
 * KDAPHPExcel_Writer_Excel5_Worksheet
 *
 * @category   KDAPHPExcel
 * @package    KDAPHPExcel_Writer_Excel5
 * @copyright  Copyright (c) 2006 - 2013 KDAPHPExcel (http://www.codeplex.com/KDAPHPExcel)
 */
class KDAPHPExcel_Writer_Excel5_Worksheet extends KDAPHPExcel_Writer_Excel5_BIFFwriter
{
	/**
	 * Formula parser
	 *
	 * @var KDAPHPExcel_Writer_Excel5_Parser
	 */
	private $_parser;
	 ...
	 * @param KDAPHPExcel_Worksheet $phpSheet
	 */
...
//		$this->_firstRowIndex = ($minR > 65535) ? 65535 : $minR;
		...
		$this->_firstColumnIndex	= KDAPHPExcel_Cell::columnIndexFromString($minC);
		$this->_lastColumnIndex		= KDAPHPExcel_Cell::columnIndexFromString($maxC);
...
//		if ($this->_firstColumnIndex > 255) $this->_firstColumnIndex = 255;
	 ...
	 *
	 * @access public
	 * @see KDAPHPExcel_Writer_Excel5_Workbook::storeWorkbook()
	 */
	function close()
	{
		...
		if (($defaultWidth = $_phpSheet->getDefaultColumnDimension()->getWidth()) < 0) {
			$defaultWidth = KDAPHPExcel_Shared_Font::getDefaultColumnWidthByFont($_phpSheet->getParent()->getDefaultStyle()->getFont());
		}
		...
		for ($i = 0; $i <= $maxCol; ++$i) {
			...
			$columnLetter = KDAPHPExcel_Cell::stringFromColumnIndex($i);
		...
		foreach ($_phpSheet->getCellCollection() as $cellID) {
			...
			$row = $cell->getRow() - 1;
			$column = KDAPHPExcel_Cell::columnIndexFromString($cell->getColumn()) - 1;
...
//			if ($row + 1 > 65536 or $column + 1 > 256) {
			...
			$cVal = $cell->getValue();
			if ($cVal instanceof KDAPHPExcel_RichText) {
				// $this->_writeString($row, $column, $cVal->getPlainText(), $xfIndex);
				$arrcRun = array();
				$str_len = KDAPHPExcel_Shared_String::CountCharacters($cVal->getPlainText(), 'UTF-8');
				$str_pos = 0;
				...
				foreach ($elements as $element) {
					...
					// Position FROM
					$str_pos += KDAPHPExcel_Shared_String::CountCharacters($element->getText(), 'UTF-8');
				}
		...
		foreach ($_phpSheet->getHyperLinkCollection() as $coordinate => $hyperlink) {
			list($column, $row) = KDAPHPExcel_Cell::coordinateFromString($coordinate);
			...
			$this->_writeUrl($row - 1, KDAPHPExcel_Cell::columnIndexFromString($column) - 1, $url);
		}
		...
		if(!empty($arrConditionalStyles)){
			...
			foreach ($arrConditionalStyles as $cellCoordinate => $conditionalStyles) {
				foreach ($conditionalStyles as $conditional) {
					if($conditional->getConditionType() == KDAPHPExcel_Style_Conditional::CONDITION_EXPRESSION
						|| $conditional->getConditionType() == KDAPHPExcel_Style_Conditional::CONDITION_CELLIS){
	...
	private function _writeBIFF8CellRangeAddressFixed($range = 'A1')
	{
		...
		$firstCellCoordinates = KDAPHPExcel_Cell::coordinateFromString($firstCell); // e.g. array(0, 1)
		$lastCellCoordinates  = KDAPHPExcel_Cell::coordinateFromString($lastCell);  // e.g. array(1, 6)

		return(pack('vvvv',
			$firstCellCoordinates[1] - 1,
			$lastCellCoordinates[1] - 1,
			KDAPHPExcel_Cell::columnIndexFromString($firstCellCoordinates[0]) - 1,
			KDAPHPExcel_Cell::columnIndexFromString($lastCellCoordinates[0]) - 1
		));
	}
	...
	private function _writeRichTextString($row, $col, $str, $xfIndex, $arrcRun){
		...
		$str = KDAPHPExcel_Shared_String::UTF8toBIFF8UnicodeShort($str, $arrcRun);
	...
	private function _writeLabel($row, $col, $str, $xfIndex)
	{
		...
		if ($strlen > $this->_xls_strmax) { // LABEL must be < 255 chars
			$str	= cp_substr($str, 0, $this->_xls_strmax);
	...
	private function _writeLabelSst($row, $col, $str, $xfIndex)
	{
		...
		$str = KDAPHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($str);
	...
	private function _writeNote($row, $col, $note)
	{
		...
		$this->_append($header . $data . cp_substr($note, 0, 2048));

		for ($i = $max_length; $i < $note_length; $i += $max_length) {
			$chunk  = cp_substr($note, $i, $max_length);
			$length = 0x0006 + cp_strlen($chunk);
			$header = pack("vv",   $record, $length);
			$data   = pack("vvv", -1, 0, cp_strlen($chunk));
	...
	private function _writeFormula($row, $col, $formula, $xfIndex, $calculatedValue)
	{
		...
		if (isset($calculatedValue)) {
			...
			if (is_bool($calculatedValue)) {
				...
				if (array_key_exists($calculatedValue, KDAPHPExcel_Cell_DataType::getErrorCodes())) {
					// Error value
		...
		try {
			...
			$formlen	= cp_strlen($formula);	// Length of the binary string
	...
	private function _writeStringRecord($stringValue)
	{
		...
		$data = KDAPHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($stringValue);
	...
	function _writeUrlWeb($row1, $col1, $row2, $col2, $url)
	{
		...
		$url_len	 = pack("V", cp_strlen($url));

		// Calculate the data length
		$length	  = 0x34 + cp_strlen($url);
	...
	function _writeUrlInternal($row1, $col1, $row2, $col2, $url)
	{
		...
		// character count
		$url_len = KDAPHPExcel_Shared_String::CountCharacters($url);
		$url_len = pack('V', $url_len);

		$url = KDAPHPExcel_Shared_String::ConvertEncoding($url, 'UTF-16LE', 'UTF-8');

		// Calculate the data length
		$length	  = 0x24 + cp_strlen($url);
	...
	function _writeUrlExternal($row1, $col1, $row2, $col2, $url)
	{
		...
		$dir_short_len = pack("V", cp_strlen($dir_short)	  );
		$dir_long_len  = pack("V", cp_strlen($dir_long)	   );
		$stream_len	= pack("V", 0);//cp_strlen($dir_long) + 0x06);
	...
	private function _writeWindow2()
	{
		...
		// no support in KDAPHPExcel for selected sheet, therefore sheet is only selected if it is the active sheet
		$fSelected	  = ($this->_phpSheet === $this->_phpSheet->getParent()->getActiveSheet()) ? 1 : 0;
		$fPaged		 = 1;					 // 2
		$fPageBreakPreview = $this->_phpSheet->getSheetView()->getView() === KDAPHPExcel_Worksheet_SheetView::SHEETVIEW_PAGE_BREAK_PREVIEW;
	...
	private function _writeSelection()
	{
		...
		$selectedCells = KDAPHPExcel_Cell::splitRange($this->_phpSheet->getSelectedCells());
		...
		list($colFirst, $rwFirst) = KDAPHPExcel_Cell::coordinateFromString($first);
		$colFirst = KDAPHPExcel_Cell::columnIndexFromString($colFirst) - 1; // base 0 column index
		--$rwFirst; // base 0 row index

		list($colLast, $rwLast) = KDAPHPExcel_Cell::coordinateFromString($last);
		$colLast = KDAPHPExcel_Cell::columnIndexFromString($colLast) - 1; // base 0 column index
	...
	private function _writeMergedCells()
	{
		...
		foreach ($mergeCells as $mergeCell) {
			...
			$range = KDAPHPExcel_Cell::splitRange($mergeCell);
			list($first, $last) = $range[0];
			list($firstColumn, $firstRow) = KDAPHPExcel_Cell::coordinateFromString($first);
			list($lastColumn, $lastRow) = KDAPHPExcel_Cell::coordinateFromString($last);

			$recordData .= pack('vvvv', $firstRow - 1, $lastRow - 1, KDAPHPExcel_Cell::columnIndexFromString($firstColumn) - 1, KDAPHPExcel_Cell::columnIndexFromString($lastColumn) - 1);

			// flush record if we have reached limit for number of merged cells, or reached final merged cell
			if ($j == $maxCountMergeCellsPerRecord or $i == $countMergeCells) {
				$recordData = pack('v', $j) . $recordData;
				$length = cp_strlen($recordData);
				$header = pack('vv', $record, $length);
	...
	private function _writeRangeProtection()
	{
		foreach ($this->_phpSheet->getProtectedCells() as $range => $password) {
			...
			$recordData .= KDAPHPExcel_Shared_String::UTF8toBIFF8UnicodeLong('p' . md5($recordData));
	...
	private function _writeExternsheet($sheetname)
	{
		...
		if ($this->_phpSheet->getTitle() == $sheetname) {
			...
			$length	= 0x02 + cp_strlen($sheetname);
			$cch	   = cp_strlen($sheetname);
	...
	private function _writePanes()
	{
		...
		if ($freezePane = $this->_phpSheet->getFreezePane()) {
			list($column, $row) = KDAPHPExcel_Cell::coordinateFromString($freezePane);
			$panes[0] = $row - 1;
			$panes[1] = KDAPHPExcel_Cell::columnIndexFromString($column) - 1;
		} else {
			// thaw panes
			return;
		}
	...
	private function _writeSetup()
	{
		...
		// Page orientation
		$fLandscape = ($this->_phpSheet->getPageSetup()->getOrientation() == KDAPHPExcel_Worksheet_PageSetup::ORIENTATION_LANDSCAPE) ?
			0x0 : 0x1;
	...
	private function _writeHeader()
	{
		...
		if (cp_strlen($this->_phpSheet->getHeaderFooter()->getOddHeader()) <= 255) {
		...
		*/
		...
		$recordData = KDAPHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($this->_phpSheet->getHeaderFooter()->getOddHeader());
		$length = cp_strlen($recordData);
	...
	private function _writeFooter()
	{
		...
		/* removing for now
		// need to fix character count (multibyte!)
		if (cp_strlen($this->_phpSheet->getHeaderFooter()->getOddFooter()) <= 255) {
		...
		*/
		...
		$recordData = KDAPHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($this->_phpSheet->getHeaderFooter()->getOddFooter());
		$length = cp_strlen($recordData);
	...
	private function _writeAutoFilterInfo(){
		...
		$rangeBounds = KDAPHPExcel_Cell::rangeBoundaries($this->_phpSheet->getAutoFilter()->getRange());
	...
	private function _writeBreaks()
	{
		...
		foreach ($this->_phpSheet->getBreaks() as $cell => $breakType) {
			// Fetch coordinates
			$coordinates = KDAPHPExcel_Cell::coordinateFromString($cell);

			// Decide what to do by the type of break
			switch ($breakType) {
				case KDAPHPExcel_Worksheet::BREAK_COLUMN:
					// Add to list of vertical breaks
					$vbreaks[] = KDAPHPExcel_Cell::columnIndexFromString($coordinates[0]) - 1;
					break;

				case KDAPHPExcel_Worksheet::BREAK_ROW:
					// Add to list of horizontal breaks
					$hbreaks[] = $coordinates[1];
					break;

				case KDAPHPExcel_Worksheet::BREAK_NONE:
				default:
					// Nothing to do
					break;
			}
	...
	function _positionImage($col_start, $row_start, $x1, $y1, $width, $height)
	{
		...
		if ($x1 >= KDAPHPExcel_Shared_Excel5::sizeCol($this->_phpSheet, KDAPHPExcel_Cell::stringFromColumnIndex($col_start))) {
			$x1 = 0;
		}
		if ($y1 >= KDAPHPExcel_Shared_Excel5::sizeRow($this->_phpSheet, $row_start + 1)) {
			$y1 = 0;
		}
		...
		while ($width >= KDAPHPExcel_Shared_Excel5::sizeCol($this->_phpSheet, KDAPHPExcel_Cell::stringFromColumnIndex($col_end))) {
			$width -= KDAPHPExcel_Shared_Excel5::sizeCol($this->_phpSheet, KDAPHPExcel_Cell::stringFromColumnIndex($col_end));
			++$col_end;
		}

		// Subtract the underlying cell heights to find the end cell of the image
		while ($height >= KDAPHPExcel_Shared_Excel5::sizeRow($this->_phpSheet, $row_end + 1)) {
			$height -= KDAPHPExcel_Shared_Excel5::sizeRow($this->_phpSheet, $row_end + 1);
			++$row_end;
		}
		...
		//
		if (KDAPHPExcel_Shared_Excel5::sizeCol($this->_phpSheet, KDAPHPExcel_Cell::stringFromColumnIndex($col_start)) == 0) {
			return;
		}
		if (KDAPHPExcel_Shared_Excel5::sizeCol($this->_phpSheet, KDAPHPExcel_Cell::stringFromColumnIndex($col_end))   == 0) {
			return;
		}
		if (KDAPHPExcel_Shared_Excel5::sizeRow($this->_phpSheet, $row_start + 1) == 0) {
			return;
		}
		if (KDAPHPExcel_Shared_Excel5::sizeRow($this->_phpSheet, $row_end + 1)   == 0) {
			return;
		}

		// Convert the pixel values to the percentage value expected by Excel
		$x1 = $x1	 / KDAPHPExcel_Shared_Excel5::sizeCol($this->_phpSheet, KDAPHPExcel_Cell::stringFromColumnIndex($col_start))   * 1024;
		$y1 = $y1	 / KDAPHPExcel_Shared_Excel5::sizeRow($this->_phpSheet, $row_start + 1)   *  256;
		$x2 = $width  / KDAPHPExcel_Shared_Excel5::sizeCol($this->_phpSheet, KDAPHPExcel_Cell::stringFromColumnIndex($col_end))	 * 1024; // Distance to right side of object
		$y2 = $height / KDAPHPExcel_Shared_Excel5::sizeRow($this->_phpSheet, $row_end + 1)	 *  256; // Distance to bottom of object
	...
	function _processBitmapGd($image) {
		...
		return array($width, $height, cp_strlen($data), $data);
	}
	...
	function _processBitmap($bitmap)
	{
		...
		if (!$bmp_fd) {
			throw new KDAPHPExcel_Writer_Exception("Couldn't import $bitmap");
		}
		...
		if (cp_strlen($data) <= 0x36) {
			throw new KDAPHPExcel_Writer_Exception("$bitmap doesn't contain enough data.\n");
		}

		// The first 2 bytes are used to identify the bitmap.
		$identity = unpack("A2ident", $data);
		if ($identity['ident'] != "BM") {
			throw new KDAPHPExcel_Writer_Exception("$bitmap doesn't appear to be a valid bitmap image.\n");
		}

		// Remove bitmap data: ID.
		$data = cp_substr($data, 2);

		// Read and remove the bitmap size. This is more reliable than reading
		// the data size at offset 0x22.
		//
		$size_array   = unpack("Vsa", cp_substr($data, 0, 4));
		$size   = $size_array['sa'];
		$data   = cp_substr($data, 4);
		$size  -= 0x36; // Subtract size of bitmap header.
		$size  += 0x0C; // Add size of BIFF header.

		// Remove bitmap data: reserved, offset, header length.
		$data = cp_substr($data, 12);

		// Read and remove the bitmap width and height. Verify the sizes.
		$width_and_height = unpack("V2", cp_substr($data, 0, 8));
		$width  = $width_and_height[1];
		$height = $width_and_height[2];
		$data   = cp_substr($data, 8);
		if ($width > 0xFFFF) {
			throw new KDAPHPExcel_Writer_Exception("$bitmap: largest image width supported is 65k.\n");
		}
		if ($height > 0xFFFF) {
			throw new KDAPHPExcel_Writer_Exception("$bitmap: largest image height supported is 65k.\n");
		}

		// Read and remove the bitmap planes and bpp data. Verify them.
		$planes_and_bitcount = unpack("v2", cp_substr($data, 0, 4));
		$data = cp_substr($data, 4);
		if ($planes_and_bitcount[2] != 24) { // Bitcount
			throw new KDAPHPExcel_Writer_Exception("$bitmap isn't a 24bit true color bitmap.\n");
		}
		if ($planes_and_bitcount[1] != 1) {
			throw new KDAPHPExcel_Writer_Exception("$bitmap: only 1 plane supported in bitmap image.\n");
		}

		// Read and remove the bitmap compression. Verify compression.
		$compression = unpack("Vcomp", cp_substr($data, 0, 4));
		$data = cp_substr($data, 4);

		//$compression = 0;
		if ($compression['comp'] != 0) {
			throw new KDAPHPExcel_Writer_Exception("$bitmap: compression not supported in bitmap image.\n");
		}

		// Remove bitmap data: data size, hres, vres, colours, imp. colours.
		$data = cp_substr($data, 20);
	 ...
	 * Get Escher object
	 *
	 * @return KDAPHPExcel_Shared_Escher
	 */
	 ...
	 * Set Escher object
	 *
	 * @param KDAPHPExcel_Shared_Escher $pValue
	 */
	public function setEscher(KDAPHPExcel_Shared_Escher $pValue = null)
	{
		$this->_escher = $pValue;
	}
	...
	private function _writeMsoDrawing()
	{
		...
		if (isset($this->_escher)) {
			$writer = new KDAPHPExcel_Writer_Excel5_Escher($this->_escher);
			$data = $writer->close();
			...
			for ($i = 1; $i <= $nm; ++$i) {
				...
				$dataChunk = cp_substr($data, $spOffsets[$i -1], $spOffsets[$i] - $spOffsets[$i - 1]);

				$length = cp_strlen($dataChunk);
				$header = pack("vv", $record, $length);
	...
	private function _writeDataValidity()
	{
		...
		if (!empty($dataValidationCollection)) {
			...
			foreach ($dataValidationCollection as $cellCoordinate => $dataValidation) {
				...
				switch ($type) {
					case KDAPHPExcel_Cell_DataValidation::TYPE_NONE:		$type = 0x00;	break;
					case KDAPHPExcel_Cell_DataValidation::TYPE_WHOLE:		$type = 0x01;	break;
					case KDAPHPExcel_Cell_DataValidation::TYPE_DECIMAL:	$type = 0x02;	break;
					case KDAPHPExcel_Cell_DataValidation::TYPE_LIST:		$type = 0x03;	break;
					case KDAPHPExcel_Cell_DataValidation::TYPE_DATE:		$type = 0x04;	break;
					case KDAPHPExcel_Cell_DataValidation::TYPE_TIME:		$type = 0x05;	break;
					case KDAPHPExcel_Cell_DataValidation::TYPE_TEXTLENGTH:	$type = 0x06;	break;
					case KDAPHPExcel_Cell_DataValidation::TYPE_CUSTOM:		$type = 0x07;	break;
				}
				$options |= $type << 0;
				...
				switch ($errorStyle) {
					case KDAPHPExcel_Cell_DataValidation::STYLE_STOP:			$errorStyle = 0x00;		break;
					case KDAPHPExcel_Cell_DataValidation::STYLE_WARNING:		$errorStyle = 0x01;		break;
					case KDAPHPExcel_Cell_DataValidation::STYLE_INFORMATION:	$errorStyle = 0x02;		break;
				}
				...
				switch ($operator) {
					case KDAPHPExcel_Cell_DataValidation::OPERATOR_BETWEEN: $operator = 0x00			;	break;
					case KDAPHPExcel_Cell_DataValidation::OPERATOR_NOTBETWEEN: $operator = 0x01		;	break;
					case KDAPHPExcel_Cell_DataValidation::OPERATOR_EQUAL: $operator = 0x02				;	break;
					case KDAPHPExcel_Cell_DataValidation::OPERATOR_NOTEQUAL: $operator = 0x03			;	break;
					case KDAPHPExcel_Cell_DataValidation::OPERATOR_GREATERTHAN: $operator = 0x04		;	break;
					case KDAPHPExcel_Cell_DataValidation::OPERATOR_LESSTHAN: $operator = 0x05			;	break;
					case KDAPHPExcel_Cell_DataValidation::OPERATOR_GREATERTHANOREQUAL: $operator = 0x06;	break;
					case KDAPHPExcel_Cell_DataValidation::OPERATOR_LESSTHANOREQUAL: $operator = 0x07	;	break;
				}
				$options |= $operator << 20;
				...
				$data .= KDAPHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($promptTitle);

				// error title
				$errorTitle = $dataValidation->getErrorTitle() !== '' ?
					$dataValidation->getErrorTitle() : chr(0);
				$data .= KDAPHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($errorTitle);

				// prompt text
				$prompt = $dataValidation->getPrompt() !== '' ?
					$dataValidation->getPrompt() : chr(0);
				$data .= KDAPHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($prompt);

				// error text
				$error = $dataValidation->getError() !== '' ?
					$dataValidation->getError() : chr(0);
				$data .= KDAPHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($error);
				...
				try {
					...
					if ($formula2 === '') {
						throw new KDAPHPExcel_Writer_Exception('No formula2');
					}
					$this->_parser->parse($formula2);
					$formula2 = $this->_parser->toReversePolish();
					$sz2 = cp_strlen($formula2);
	...
	/**
	 * Write CFRule Record
	 * @param KDAPHPExcel_Style_Conditional $conditional
	 */
	private function _writeCFRule(KDAPHPExcel_Style_Conditional $conditional){
		...
		if($conditional->getConditionType() == KDAPHPExcel_Style_Conditional::CONDITION_EXPRESSION){
			$type = 0x02;
			$operatorType = 0x00;
		} else if($conditional->getConditionType() == KDAPHPExcel_Style_Conditional::CONDITION_CELLIS){
			$type = 0x01;

			switch ($conditional->getOperatorType()){
				case KDAPHPExcel_Style_Conditional::OPERATOR_NONE:
					$operatorType = 0x00;
					break;
				case KDAPHPExcel_Style_Conditional::OPERATOR_EQUAL:
					$operatorType = 0x03;
					break;
				case KDAPHPExcel_Style_Conditional::OPERATOR_GREATERTHAN:
					$operatorType = 0x05;
					break;
				case KDAPHPExcel_Style_Conditional::OPERATOR_GREATERTHANOREQUAL:
					$operatorType = 0x07;
					break;
				case KDAPHPExcel_Style_Conditional::OPERATOR_LESSTHAN:
					$operatorType = 0x06;
					break;
				case KDAPHPExcel_Style_Conditional::OPERATOR_LESSTHANOREQUAL:
					$operatorType = 0x08;
					break;
				case KDAPHPExcel_Style_Conditional::OPERATOR_NOTEQUAL:
					$operatorType = 0x04;
					break;
				case KDAPHPExcel_Style_Conditional::OPERATOR_BETWEEN:
					$operatorType = 0x01;
					break;
		...
		if($numConditions == 1){
			...
			$operand2 = null;
		} else if($numConditions == 2 && ($conditional->getOperatorType() == KDAPHPExcel_Style_Conditional::OPERATOR_BETWEEN)){
			$szValue1 = ($arrConditions[0] <= 65535 ? 3 : 0x0000);
		...
		// Border
		$bBorderLeft = ($conditional->getStyle()->getBorders()->getLeft()->getColor()->getARGB() == KDAPHPExcel_Style_Color::COLOR_BLACK
						&& $conditional->getStyle()->getBorders()->getLeft()->getBorderStyle() == KDAPHPExcel_Style_Border::BORDER_NONE ? 1 : 0);
		$bBorderRight = ($conditional->getStyle()->getBorders()->getRight()->getColor()->getARGB() == KDAPHPExcel_Style_Color::COLOR_BLACK
						&& $conditional->getStyle()->getBorders()->getRight()->getBorderStyle() == KDAPHPExcel_Style_Border::BORDER_NONE ? 1 : 0);
		$bBorderTop = ($conditional->getStyle()->getBorders()->getTop()->getColor()->getARGB() == KDAPHPExcel_Style_Color::COLOR_BLACK
						&& $conditional->getStyle()->getBorders()->getTop()->getBorderStyle() == KDAPHPExcel_Style_Border::BORDER_NONE ? 1 : 0);
		$bBorderBottom = ($conditional->getStyle()->getBorders()->getBottom()->getColor()->getARGB() == KDAPHPExcel_Style_Color::COLOR_BLACK
						&& $conditional->getStyle()->getBorders()->getBottom()->getBorderStyle() == KDAPHPExcel_Style_Border::BORDER_NONE ? 1 : 0);
		...
		if($bFormatFont == 1){
			...
			if($conditional->getStyle()->getFont()->getName() == null){
				...
				$dataBlockFont = KDAPHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($conditional->getStyle()->getFont()->getName());
			}
			// Font Size
			...
			switch ($conditional->getStyle()->getFont()->getUnderline()){
				case KDAPHPExcel_Style_Font::UNDERLINE_NONE             : $dataBlockFont .= pack('C', 0x00); $fontUnderline = 0; break;
				case KDAPHPExcel_Style_Font::UNDERLINE_DOUBLE           : $dataBlockFont .= pack('C', 0x02); $fontUnderline = 0; break;
				case KDAPHPExcel_Style_Font::UNDERLINE_DOUBLEACCOUNTING : $dataBlockFont .= pack('C', 0x22); $fontUnderline = 0; break;
				case KDAPHPExcel_Style_Font::UNDERLINE_SINGLE           : $dataBlockFont .= pack('C', 0x01); $fontUnderline = 0; break;
				case KDAPHPExcel_Style_Font::UNDERLINE_SINGLEACCOUNTING : $dataBlockFont .= pack('C', 0x21); $fontUnderline = 0; break;
		...
		if($bFormatAlign == 1){
			...
			switch ($conditional->getStyle()->getAlignment()->getHorizontal()){
				case KDAPHPExcel_Style_Alignment::HORIZONTAL_GENERAL 			: $blockAlign = 0; break;
				case KDAPHPExcel_Style_Alignment::HORIZONTAL_LEFT				: $blockAlign = 1; break;
				case KDAPHPExcel_Style_Alignment::HORIZONTAL_RIGHT				: $blockAlign = 3; break;
				case KDAPHPExcel_Style_Alignment::HORIZONTAL_CENTER			: $blockAlign = 2; break;
				case KDAPHPExcel_Style_Alignment::HORIZONTAL_CENTER_CONTINUOUS	: $blockAlign = 6; break;
				case KDAPHPExcel_Style_Alignment::HORIZONTAL_JUSTIFY			: $blockAlign = 5; break;
			}
			...
			switch ($conditional->getStyle()->getAlignment()->getVertical()){
				case KDAPHPExcel_Style_Alignment::VERTICAL_BOTTOM 				: $blockAlign = 2 << 4; break;
				case KDAPHPExcel_Style_Alignment::VERTICAL_TOP					: $blockAlign = 0 << 4; break;
				case KDAPHPExcel_Style_Alignment::VERTICAL_CENTER				: $blockAlign = 1 << 4; break;
				case KDAPHPExcel_Style_Alignment::VERTICAL_JUSTIFY				: $blockAlign = 3 << 4; break;
			}
			$blockAlign |= 0 << 7;
		...
		if($bFormatBorder == 1){
			...
			switch ($conditional->getStyle()->getBorders()->getLeft()->getBorderStyle()){
				case KDAPHPExcel_Style_Border::BORDER_NONE              : $blockLineStyle |= 0x00; break;
				case KDAPHPExcel_Style_Border::BORDER_THIN              : $blockLineStyle |= 0x01; break;
				case KDAPHPExcel_Style_Border::BORDER_MEDIUM            : $blockLineStyle |= 0x02; break;
				case KDAPHPExcel_Style_Border::BORDER_DASHED            : $blockLineStyle |= 0x03; break;
				case KDAPHPExcel_Style_Border::BORDER_DOTTED            : $blockLineStyle |= 0x04; break;
				case KDAPHPExcel_Style_Border::BORDER_THICK             : $blockLineStyle |= 0x05; break;
				case KDAPHPExcel_Style_Border::BORDER_DOUBLE            : $blockLineStyle |= 0x06; break;
				case KDAPHPExcel_Style_Border::BORDER_HAIR              : $blockLineStyle |= 0x07; break;
				case KDAPHPExcel_Style_Border::BORDER_MEDIUMDASHED      : $blockLineStyle |= 0x08; break;
				case KDAPHPExcel_Style_Border::BORDER_DASHDOT           : $blockLineStyle |= 0x09; break;
				case KDAPHPExcel_Style_Border::BORDER_MEDIUMDASHDOT     : $blockLineStyle |= 0x0A; break;
				case KDAPHPExcel_Style_Border::BORDER_DASHDOTDOT        : $blockLineStyle |= 0x0B; break;
				case KDAPHPExcel_Style_Border::BORDER_MEDIUMDASHDOTDOT  : $blockLineStyle |= 0x0C; break;
				case KDAPHPExcel_Style_Border::BORDER_SLANTDASHDOT      : $blockLineStyle |= 0x0D; break;
			}
			switch ($conditional->getStyle()->getBorders()->getRight()->getBorderStyle()){
				case KDAPHPExcel_Style_Border::BORDER_NONE              : $blockLineStyle |= 0x00 << 4; break;
				case KDAPHPExcel_Style_Border::BORDER_THIN              : $blockLineStyle |= 0x01 << 4; break;
				case KDAPHPExcel_Style_Border::BORDER_MEDIUM            : $blockLineStyle |= 0x02 << 4; break;
				case KDAPHPExcel_Style_Border::BORDER_DASHED            : $blockLineStyle |= 0x03 << 4; break;
				case KDAPHPExcel_Style_Border::BORDER_DOTTED            : $blockLineStyle |= 0x04 << 4; break;
				case KDAPHPExcel_Style_Border::BORDER_THICK             : $blockLineStyle |= 0x05 << 4; break;
				case KDAPHPExcel_Style_Border::BORDER_DOUBLE            : $blockLineStyle |= 0x06 << 4; break;
				case KDAPHPExcel_Style_Border::BORDER_HAIR              : $blockLineStyle |= 0x07 << 4; break;
				case KDAPHPExcel_Style_Border::BORDER_MEDIUMDASHED      : $blockLineStyle |= 0x08 << 4; break;
				case KDAPHPExcel_Style_Border::BORDER_DASHDOT           : $blockLineStyle |= 0x09 << 4; break;
				case KDAPHPExcel_Style_Border::BORDER_MEDIUMDASHDOT     : $blockLineStyle |= 0x0A << 4; break;
				case KDAPHPExcel_Style_Border::BORDER_DASHDOTDOT        : $blockLineStyle |= 0x0B << 4; break;
				case KDAPHPExcel_Style_Border::BORDER_MEDIUMDASHDOTDOT  : $blockLineStyle |= 0x0C << 4; break;
				case KDAPHPExcel_Style_Border::BORDER_SLANTDASHDOT      : $blockLineStyle |= 0x0D << 4; break;
			}
			switch ($conditional->getStyle()->getBorders()->getTop()->getBorderStyle()){
				case KDAPHPExcel_Style_Border::BORDER_NONE              : $blockLineStyle |= 0x00 << 8; break;
				case KDAPHPExcel_Style_Border::BORDER_THIN              : $blockLineStyle |= 0x01 << 8; break;
				case KDAPHPExcel_Style_Border::BORDER_MEDIUM            : $blockLineStyle |= 0x02 << 8; break;
				case KDAPHPExcel_Style_Border::BORDER_DASHED            : $blockLineStyle |= 0x03 << 8; break;
				case KDAPHPExcel_Style_Border::BORDER_DOTTED            : $blockLineStyle |= 0x04 << 8; break;
				case KDAPHPExcel_Style_Border::BORDER_THICK             : $blockLineStyle |= 0x05 << 8; break;
				case KDAPHPExcel_Style_Border::BORDER_DOUBLE            : $blockLineStyle |= 0x06 << 8; break;
				case KDAPHPExcel_Style_Border::BORDER_HAIR              : $blockLineStyle |= 0x07 << 8; break;
				case KDAPHPExcel_Style_Border::BORDER_MEDIUMDASHED      : $blockLineStyle |= 0x08 << 8; break;
				case KDAPHPExcel_Style_Border::BORDER_DASHDOT           : $blockLineStyle |= 0x09 << 8; break;
				case KDAPHPExcel_Style_Border::BORDER_MEDIUMDASHDOT     : $blockLineStyle |= 0x0A << 8; break;
				case KDAPHPExcel_Style_Border::BORDER_DASHDOTDOT        : $blockLineStyle |= 0x0B << 8; break;
				case KDAPHPExcel_Style_Border::BORDER_MEDIUMDASHDOTDOT  : $blockLineStyle |= 0x0C << 8; break;
				case KDAPHPExcel_Style_Border::BORDER_SLANTDASHDOT      : $blockLineStyle |= 0x0D << 8; break;
			}
			switch ($conditional->getStyle()->getBorders()->getBottom()->getBorderStyle()){
				case KDAPHPExcel_Style_Border::BORDER_NONE              : $blockLineStyle |= 0x00 << 12; break;
				case KDAPHPExcel_Style_Border::BORDER_THIN              : $blockLineStyle |= 0x01 << 12; break;
				case KDAPHPExcel_Style_Border::BORDER_MEDIUM            : $blockLineStyle |= 0x02 << 12; break;
				case KDAPHPExcel_Style_Border::BORDER_DASHED            : $blockLineStyle |= 0x03 << 12; break;
				case KDAPHPExcel_Style_Border::BORDER_DOTTED            : $blockLineStyle |= 0x04 << 12; break;
				case KDAPHPExcel_Style_Border::BORDER_THICK             : $blockLineStyle |= 0x05 << 12; break;
				case KDAPHPExcel_Style_Border::BORDER_DOUBLE            : $blockLineStyle |= 0x06 << 12; break;
				case KDAPHPExcel_Style_Border::BORDER_HAIR              : $blockLineStyle |= 0x07 << 12; break;
				case KDAPHPExcel_Style_Border::BORDER_MEDIUMDASHED      : $blockLineStyle |= 0x08 << 12; break;
				case KDAPHPExcel_Style_Border::BORDER_DASHDOT           : $blockLineStyle |= 0x09 << 12; break;
				case KDAPHPExcel_Style_Border::BORDER_MEDIUMDASHDOT     : $blockLineStyle |= 0x0A << 12; break;
				case KDAPHPExcel_Style_Border::BORDER_DASHDOTDOT        : $blockLineStyle |= 0x0B << 12; break;
				case KDAPHPExcel_Style_Border::BORDER_MEDIUMDASHDOTDOT  : $blockLineStyle |= 0x0C << 12; break;
				case KDAPHPExcel_Style_Border::BORDER_SLANTDASHDOT      : $blockLineStyle |= 0x0D << 12; break;
			}
			...
			switch ($conditional->getStyle()->getBorders()->getDiagonal()->getBorderStyle()){
				case KDAPHPExcel_Style_Border::BORDER_NONE              : $blockColor |= 0x00 << 21; break;
				case KDAPHPExcel_Style_Border::BORDER_THIN              : $blockColor |= 0x01 << 21; break;
				case KDAPHPExcel_Style_Border::BORDER_MEDIUM            : $blockColor |= 0x02 << 21; break;
				case KDAPHPExcel_Style_Border::BORDER_DASHED            : $blockColor |= 0x03 << 21; break;
				case KDAPHPExcel_Style_Border::BORDER_DOTTED            : $blockColor |= 0x04 << 21; break;
				case KDAPHPExcel_Style_Border::BORDER_THICK             : $blockColor |= 0x05 << 21; break;
				case KDAPHPExcel_Style_Border::BORDER_DOUBLE            : $blockColor |= 0x06 << 21; break;
				case KDAPHPExcel_Style_Border::BORDER_HAIR              : $blockColor |= 0x07 << 21; break;
				case KDAPHPExcel_Style_Border::BORDER_MEDIUMDASHED      : $blockColor |= 0x08 << 21; break;
				case KDAPHPExcel_Style_Border::BORDER_DASHDOT           : $blockColor |= 0x09 << 21; break;
				case KDAPHPExcel_Style_Border::BORDER_MEDIUMDASHDOT     : $blockColor |= 0x0A << 21; break;
				case KDAPHPExcel_Style_Border::BORDER_DASHDOTDOT        : $blockColor |= 0x0B << 21; break;
				case KDAPHPExcel_Style_Border::BORDER_MEDIUMDASHDOTDOT  : $blockColor |= 0x0C << 21; break;
				case KDAPHPExcel_Style_Border::BORDER_SLANTDASHDOT      : $blockColor |= 0x0D << 21; break;
			}
		...
		if($bFormatFill == 1){
			...
			switch ($conditional->getStyle()->getFill()->getFillType()){
				case KDAPHPExcel_Style_Fill::FILL_NONE						: $blockFillPatternStyle = 0x00; break;
				case KDAPHPExcel_Style_Fill::FILL_SOLID					: $blockFillPatternStyle = 0x01; break;
				case KDAPHPExcel_Style_Fill::FILL_PATTERN_MEDIUMGRAY		: $blockFillPatternStyle = 0x02; break;
				case KDAPHPExcel_Style_Fill::FILL_PATTERN_DARKGRAY			: $blockFillPatternStyle = 0x03; break;
				case KDAPHPExcel_Style_Fill::FILL_PATTERN_LIGHTGRAY		: $blockFillPatternStyle = 0x04; break;
				case KDAPHPExcel_Style_Fill::FILL_PATTERN_DARKHORIZONTAL	: $blockFillPatternStyle = 0x05; break;
				case KDAPHPExcel_Style_Fill::FILL_PATTERN_DARKVERTICAL		: $blockFillPatternStyle = 0x06; break;
				case KDAPHPExcel_Style_Fill::FILL_PATTERN_DARKDOWN			: $blockFillPatternStyle = 0x07; break;
				case KDAPHPExcel_Style_Fill::FILL_PATTERN_DARKUP			: $blockFillPatternStyle = 0x08; break;
				case KDAPHPExcel_Style_Fill::FILL_PATTERN_DARKGRID			: $blockFillPatternStyle = 0x09; break;
				case KDAPHPExcel_Style_Fill::FILL_PATTERN_DARKTRELLIS		: $blockFillPatternStyle = 0x0A; break;
				case KDAPHPExcel_Style_Fill::FILL_PATTERN_LIGHTHORIZONTAL	: $blockFillPatternStyle = 0x0B; break;
				case KDAPHPExcel_Style_Fill::FILL_PATTERN_LIGHTVERTICAL	: $blockFillPatternStyle = 0x0C; break;
				case KDAPHPExcel_Style_Fill::FILL_PATTERN_LIGHTDOWN		: $blockFillPatternStyle = 0x0D; break;
				case KDAPHPExcel_Style_Fill::FILL_PATTERN_LIGHTUP			: $blockFillPatternStyle = 0x0E; break;
				case KDAPHPExcel_Style_Fill::FILL_PATTERN_LIGHTGRID		: $blockFillPatternStyle = 0x0F; break;
				case KDAPHPExcel_Style_Fill::FILL_PATTERN_LIGHTTRELLIS		: $blockFillPatternStyle = 0x10; break;
				case KDAPHPExcel_Style_Fill::FILL_PATTERN_GRAY125			: $blockFillPatternStyle = 0x11; break;
				case KDAPHPExcel_Style_Fill::FILL_PATTERN_GRAY0625			: $blockFillPatternStyle = 0x12; break;
				case KDAPHPExcel_Style_Fill::FILL_GRADIENT_LINEAR			: $blockFillPatternStyle = 0x00; break;	// does not exist in BIFF8
				case KDAPHPExcel_Style_Fill::FILL_GRADIENT_PATH			: $blockFillPatternStyle = 0x00; break;	// does not exist in BIFF8
		...
		if($bFormatProt == 1){
			...
			if($conditional->getStyle()->getProtection()->getLocked() == KDAPHPExcel_Style_Protection::PROTECTION_PROTECTED){
				$dataBlockProtection = 1;
			}
			if($conditional->getStyle()->getProtection()->getHidden() == KDAPHPExcel_Style_Protection::PROTECTION_PROTECTED){
		...
		$header	  = pack('vv', $record, cp_strlen($data));
		$this->_append($header . $data);
	}
		...
		foreach ($this->_phpSheet->getConditionalStylesCollection() as $cellCoordinate => $conditionalStyles) {
			foreach ($conditionalStyles as $conditional) {
				if($conditional->getConditionType() == KDAPHPExcel_Style_Conditional::CONDITION_EXPRESSION
					...
					// Cells
					$arrCoord = KDAPHPExcel_Cell::coordinateFromString($cellCoordinate);
					if(!is_numeric($arrCoord[0])){
						$arrCoord[0] = KDAPHPExcel_Cell::columnIndexFromString($arrCoord[0]);
					}
