		...
		if (preg_match('/^(\[\$[A-Z]*-[0-9A-F]*\])*[hmsdy]/i', $format)) { // datetime format
			...
			if ($format === self::FORMAT_CURRENCY_EUR_SIMPLE) {
				...
				if (preg_match('/#?.*\?\/\?/', $format, $m)) {
					...
					if(preg_match('/^(\[\$.*\])?((#+(\xC2\xA0|\s)*)+0+)(\.0+)?$/', $format, $m))
					{
						$v1 = $m[2];
						$v2l = strlen(current(explode('.', $value)));
						while(strpos($v1, '#')===0 && strlen(preg_replace('/(\xC2\xA0|\s)/', '', $v1)) > $v2l)
						{
							$v1 = preg_replace('/(^(\xC2\xA0|\s)+|(\xC2\xA0|\s)+$)/s', '', (substr($v1, 1)));
						}
						if($v1!=$m[2]) $format = $m[1].$v1.$m[5];
					}
				...
				elseif (preg_match('/\[\$(.*)\]/', $format, $m)) {
					...
					if($currencyCode=="\xE2") $currencyCode = "₽";
					$value = preg_replace('/\[\$([^\]]*)\]/',$currencyCode,$value);
				}
			}
		}
