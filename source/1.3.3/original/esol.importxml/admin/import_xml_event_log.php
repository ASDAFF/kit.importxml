...
while($db_res = $rsData->NavNext(true, "a_"))
{
	...
	if($a_TYPE)
	{
		if($a_TYPE=='ELEMENT_NOT_FOUND')
		{
			...
			$a_FIELDS = '<b>'.GetMessage("ESOL_IX_EVENTLOG_FILTER_FIELDS").'</b>'.$logger->GetElementDescriptionArray($arFieldsFilter);
	...
	$row->AddViewField("TYPE", GetMessage("ESOL_IX_EVENTLOG_IBLOCK_".$a_TYPE));
	...
	$arActions = array();
	$arActions[] = array("ICON"=>"delete", "TEXT"=>GetMessage("ESOL_IX_LOG_RECORD_DELETE"), "ACTION"=>"if(confirm('".GetMessageJS('ESOL_IX_LOG_RECORD_DELETE_CONFIRM')."')) ".$lAdmin->ActionDoGroup($a_ID, "delete"));
