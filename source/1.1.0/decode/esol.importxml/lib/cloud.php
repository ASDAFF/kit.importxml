...
class Cloud
{
	...
	public function GdriveGetAccessToken(&$arFile, $id, $type='file')
	{
		...
		if($type=='folder') $apiPath = 'https://www.googleapis.com/drive/v3/files/?q="'.$id.'"+in+parents&fields=files(id,name)';
		else $apiPath = 'https://www.googleapis.com/drive/v3/files/'.$id.'?fields=id,webContentLink,name';
		if($accessToken)
		{
			$ob = new \Bitrix\Main\Web\HttpClient(array('socketTimeout'=>10, 'disableSslVerification'=>false));
