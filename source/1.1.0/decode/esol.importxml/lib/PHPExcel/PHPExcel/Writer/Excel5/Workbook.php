...
/**
 * KDAPHPExcel_Writer_Excel5_Workbook
 *
 * @category   KDAPHPExcel
 * @package    KDAPHPExcel_Writer_Excel5
 * @copyright  Copyright (c) 2006 - 2013 KDAPHPExcel (http://www.codeplex.com/KDAPHPExcel)
 */
class KDAPHPExcel_Writer_Excel5_Workbook extends KDAPHPExcel_Writer_Excel5_BIFFwriter
{
	/**
	 * Formula parser
	 *
	...
	/**
	 * XF Writers
	 * @var KDAPHPExcel_Writer_Excel5_Xf[]
	 */
	 ...
	 * Fonts writers
	 *
	 * @var KDAPHPExcel_Writer_Excel5_Font[]
	 */
	 ...
	 * Class constructor
	 *
	 * @param KDAPHPExcel	$phpExcel		The Workbook
	 ...
	 */
	public function __construct(KDAPHPExcel $phpExcel = null,
	 ...
	 */
	public function _addFont(KDAPHPExcel_Style_Font $font)
	{
		...
		if(isset($this->_addedFonts[$fontHashCode])){
			...
			$fontWriter = new KDAPHPExcel_Writer_Excel5_Font($font);
		...
		if (!isset($this->_colors[$rgb])) {
			if (count($this->_colors) < 57) {
				...
				$this->_palette[$colorIndex] =
					array(
						hexdec(cp_substr($rgb, 0, 2)),
						hexdec(cp_substr($rgb, 2, 2)),
						hexdec(cp_substr($rgb, 4)),
						0
					);
	...
	function _calcSheetOffsets()
	{
		...
		foreach ($this->_phpExcel->getWorksheetIterator() as $sheet) {
			$offset += $boundsheet_length + cp_strlen(KDAPHPExcel_Shared_String::UTF8toBIFF8UnicodeShort($sheet->getTitle()));
		}
	...
	private function _writeNames()
	{
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			...
			if ($sheetSetup->isPrintAreaSet()) {
				// Print area
				$printArea = KDAPHPExcel_Cell::splitRange($sheetSetup->getPrintArea());
				$printArea = $printArea[0];
				$printArea[0] = KDAPHPExcel_Cell::coordinateFromString($printArea[0]);
				$printArea[1] = KDAPHPExcel_Cell::coordinateFromString($printArea[1]);

				$print_rowmin = $printArea[0][1] - 1;
				$print_rowmax = $printArea[1][1] - 1;
				$print_colmin = KDAPHPExcel_Cell::columnIndexFromString($printArea[0][0]) - 1;
				$print_colmax = KDAPHPExcel_Cell::columnIndexFromString($printArea[1][0]) - 1;
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			...
			if ($sheetSetup->isColumnsToRepeatAtLeftSet() && $sheetSetup->isRowsToRepeatAtTopSet()) {
				...
				$colmin = KDAPHPExcel_Cell::columnIndexFromString($repeat[0]) - 1;
				$colmax = KDAPHPExcel_Cell::columnIndexFromString($repeat[1]) - 1;
	...
	private function _writeAllDefinedNamesBiff8()
	{
		...
		if (count($this->_phpExcel->getNamedRanges()) > 0) {
			...
			foreach ($namedRanges as $namedRange) {
				...
				$range = KDAPHPExcel_Cell::splitRange($namedRange->getRange());
				for ($i = 0; $i < count($range); $i++) {
					$range[$i][0] = '\'' . str_replace("'", "''", $namedRange->getWorksheet()->getTitle()) . '\'!' . KDAPHPExcel_Cell::absoluteCoordinate($range[$i][0]);
					if (isset($range[$i][1])) {
						$range[$i][1] = KDAPHPExcel_Cell::absoluteCoordinate($range[$i][1]);
					}
				}
				$range = KDAPHPExcel_Cell::buildRange($range); // e.g. Sheet1!$A$1:$B$2
				...
				try {
					...
					if (isset($formulaData[0]) and ($formulaData[0] == "\x7A" or $formulaData[0] == "\x5A")) {
						$formulaData = "\x3A" . cp_substr($formulaData, 1);
					}
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			...
			if ($sheetSetup->isPrintAreaSet()) {
				...
				for ($j = 0; $j < $countPrintArea; ++$j) {
					...
					$printAreaRect[0] = KDAPHPExcel_Cell::coordinateFromString($printAreaRect[0]);
					$printAreaRect[1] = KDAPHPExcel_Cell::coordinateFromString($printAreaRect[1]);

					$print_rowmin = $printAreaRect[0][1] - 1;
					$print_rowmax = $printAreaRect[1][1] - 1;
					$print_colmin = KDAPHPExcel_Cell::columnIndexFromString($printAreaRect[0][0]) - 1;
					$print_colmax = KDAPHPExcel_Cell::columnIndexFromString($printAreaRect[1][0]) - 1;
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			...
			if(!empty($autoFilterRange)) {
				$rangeBounds = KDAPHPExcel_Cell::rangeBoundaries($autoFilterRange);
	...
	private function _writeDefinedNameBiff8($name, $formulaData, $sheetIndex = 0, $isBuiltIn = false)
	{
		...
		$nlen = KDAPHPExcel_Shared_String::CountCharacters($name);

		// name with stripped length field
		$name = cp_substr(KDAPHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($name), 2);

		// size of the formula (in bytes)
		$sz = cp_strlen($formulaData);
	...
	private function _writeWindow1()
	{
		...
		// not supported by KDAPHPExcel, so there is only one selected sheet, the active
		$ctabsel   = 1;       // Number of workbook tabs selected

		$wTabRatio = 0x0258;                 // Tab to scrollbar ratio

		// not supported by KDAPHPExcel, set to 0
	 ...
	 *
	 * @param KDAPHPExcel_Worksheet  $sheet Worksheet name
	...
	private function _writeBoundsheet($sheet, $offset)
	{
		...
		switch ($sheet->getSheetState()) {
			case KDAPHPExcel_Worksheet::SHEETSTATE_VISIBLE:	$ss = 0x00; break;
			case KDAPHPExcel_Worksheet::SHEETSTATE_HIDDEN:		$ss = 0x01; break;
			case KDAPHPExcel_Worksheet::SHEETSTATE_VERYHIDDEN:	$ss = 0x02; break;
		...
		$data .= KDAPHPExcel_Shared_String::UTF8toBIFF8UnicodeShort($sheetname);
	...
	private function _writeNumFormat($format, $ifmt)
	{
		...
		$numberFormatString = KDAPHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($format);
		$length    = 2 + cp_strlen($numberFormatString);      // Number of bytes to follow
	...
	private function _writeDatemode()
	{
		...
		$f1904     = (KDAPHPExcel_Shared_Date::getExcelCalendar() == KDAPHPExcel_Shared_Date::CALENDAR_MAC_1904) ?
	...
	{
		$record      = 0x0017;                     // Record identifier
		$length      = 0x02 + cp_strlen($sheetname);  // Number of bytes to follow

		$cch         = cp_strlen($sheetname);         // Length of sheet name
	...
	private function _writeSharedStringsTable()
	{
		...
		foreach (array_keys($this->_str_table) as $string) {
			...
			while ($finished === false) {
				...
				if (cp_strlen($recordData) + cp_strlen($string) <= $continue_limit) {
					...
					$space_remaining = $continue_limit - cp_strlen($recordData);
					...
					// 2. space remaining is greater than or equal to minimum space needed
						...
						if ( $encoding == 1 && (cp_strlen($string) - $space_remaining) % 2 == 1 ) {
							--$effective_space_remaining;
						}

						// one block fininshed, store the block data
						$recordData .= cp_substr($string, 0, $effective_space_remaining);

						$string = cp_substr($string, $effective_space_remaining); // for next cycle in while loop
						$recordDatas[] = $recordData;
		...
		foreach ($recordDatas as $i => $recordData) {
			...
			$header = pack("vv", $record, cp_strlen($recordData));
