...
class KDAPHPExcel_Shared_OLE
{
	...
	*/
	public function read($file)
	{
		$fh = fopen($file, "r");
		if (!$fh) {
			throw new KDAPHPExcel_Reader_Exception("Can't open file $file");
		}
		$this->_file_handle = $fh;

		$signature = fread($fh, 8);
		if ("\xD0\xCF\x11\xE0\xA1\xB1\x1A\xE1" != $signature) {
			throw new KDAPHPExcel_Reader_Exception("File doesn't seem to be an OLE container.");
		}
		fseek($fh, 28);
		if (fread($fh, 2) != "\xFE\xFF") {
			// This shouldn't be a problem in practice
			throw new KDAPHPExcel_Reader_Exception("Only Little-Endian encoding is supported.");
		}
	...
	* use KDAPHPExcel_Shared_OLE_PPS_File::getStream().
	...
	public function getStream($blockIdOrPps)
	{
		...
		if (!$isRegistered) {
			stream_wrapper_register('ole-chainedblockstream',
				'KDAPHPExcel_Shared_OLE_ChainedBlockStream');
			$isRegistered = true;
		}
		...
		if ($blockIdOrPps instanceof KDAPHPExcel_Shared_OLE_PPS) {
	...
	*/
	public function _readPpsWks($blockId)
	{
		...
		for ($pos = 0; ; $pos += 128) {
			...
			$nameUtf16 = cp_substr($nameUtf16, 0, $nameLength - 2);
			// Simple conversion from UTF-16LE to ISO-8859-1
			$name = str_replace("\x00", "", $nameUtf16);
			$type = self::_readInt1($fh);
			switch ($type) {
			case self::OLE_PPS_TYPE_ROOT:
				$pps = new KDAPHPExcel_Shared_OLE_PPS_Root(null, null, array());
				$this->root = $pps;
				break;
			case self::OLE_PPS_TYPE_DIR:
				$pps = new KDAPHPExcel_Shared_OLE_PPS(null, null, null, null, null,
								   null, null, null, null, array());
				break;
			case self::OLE_PPS_TYPE_FILE:
				$pps = new KDAPHPExcel_Shared_OLE_PPS_File($name);
				break;
			default:
				continue;
			}
	...
	*/
	...
	{
		if (cp_strlen($string) != 8) {
			return new PEAR_Error("Expecting 8 byte string");
		}

		// factor used for separating numbers into 4 bytes parts
		$factor = pow(2,32);
		list(, $high_part) = unpack('V', cp_substr($string, 4, 4));
		list(, $low_part) = unpack('V', cp_substr($string, 0, 4));
