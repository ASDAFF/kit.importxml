...
if($_POST['action']=='save' && $_POST['EMAIL_SETTINGS'])
{
	...
	echo '$(".kit-ix-file-choose input[name=\"SETTINGS_DEFAULT[EMAIL_DATA_FILE]\"]").val("'.htmlspecialcharsex(base64_encode(serialize($_POST['EMAIL_SETTINGS']))).'");';
...
<form action="<?echo $APPLICATION->GetCurUri();?>" method="post" enctype="multipart/form-data" name="field_settings">
	...
	<?//ShowPostData($_POST);?>
	<table width="100%" class="kit-ix-list-settings kit-ix-email-settings">
		<col width="50%">
		<col width="50%">
		<tr class="heading">
			<td colspan="2">
		...
		<tr>
			...
			<td class="adm-detail-content-cell-r">
				<input type="password" name="EMAIL_SETTINGS[PASSWORD]" value="<?echo htmlspecialcharsex($EMAIL_SETTINGS['PASSWORD'])?>"> <?/*?><a href="javascript:void(0)" onclick="var inp = $(this).prev('input')[0]; inp.type=(inp.type=='password' ? 'text' : 'password');"><?echo GetMessage("KIT_IX_ECON_SHOW_HIDE");?></a><?*/?>
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td class="adm-detail-content-cell-r">
				...
				<input type="checkbox" name="EMAIL_SETTINGS[FILENAME_REGEXP]" value="Y" <?if($EMAIL_SETTINGS['FILENAME_REGEXP']=='Y'){echo 'checked';}?> id="email_filename_regexp"> <label for="email_filename_regexp"><?echo GetMessage("KIT_IX_ECON_FILENAME_REGEXP");?></label>
			</td>
		</tr>
		<tr>
		...
		<tr>
			<td class="adm-detail-content-cell-l"><?echo GetMessage("KIT_IX_ECON_SET_SEEN_OLD_MAIL");?>:</td>
			<td class="adm-detail-content-cell-r">
		...
		<tr>
			<td class="adm-detail-content-cell-l"><?echo GetMessage("KIT_IX_ECON_DELETE_OLD_MAIL");?>:</td>
			<td class="adm-detail-content-cell-r">
