...
if($REQUEST_METHOD == "POST" && check_bitrix_sessid())
{
	...
	{
		$rb = new \Bitrix\KitImportxml\ImporterRollback($_REQUEST);
		...
		if($arResult['STATUS']=='PROGRESS')
		{
			$message = new CAdminMessage(array(
				"MESSAGE" => GetMessage("KIT_IX_ROLLBACK_IN_PROGRESS"),
				"DETAILS" => GetMessage("KIT_IX_ROLLBACK_TOTAL") . " <span id=\"some_left\"><b>" . $arResult['NS']["currentCount"] . "</b></span><br>#PROGRESS_BAR#",
				"HTML" => true,
				"TYPE" => "PROGRESS",
		...
		{
			$message = new CAdminMessage(array(
				"MESSAGE" => GetMessage("KIT_IX_ROLLBACK_COMPLETE"),
				"DETAILS" => GetMessage("KIT_IX_ROLLBACK_TOTAL") . " <b>" . $arResult['NS']["currentCount"] . "</b>",
				"HTML" => true,
				"TYPE" => "OK",
			));
		}
...
/////////////////////////////////////////////////////////////////////
$APPLICATION->SetTitle(GetMessage("KIT_IX_ROLLBACK_PAGE_TITLE"));
...
$aMenu = array(
	array(
		"TEXT" => GetMessage("KIT_IX_BACK_TO_PROFILES"),
		"ICON" => "btn_list",
...
<form method="POST" action="<?echo $sDocPath ?>?lang=<?echo LANG ?>" ENCTYPE="multipart/form-data" name="rollback" id="kit-ix-rollback">
...
$aTabs = array(
	array(
		"DIV" => "edit1",
		"TAB" => GetMessage("KIT_IX_ROLLBACK_TAB1") ,
		"ICON" => "iblock",
		"TITLE" => sprintf(GetMessage("KIT_IX_ROLLBACK_TAB1_ALT"), (isset($arProfile['NAME']) ? $arProfile['NAME'] : '')),
	)
);
...
<?$tabControl->BeginNextTab();?>
	...
	?>
		<tr>
			<td width="50%"><?echo GetMessage("KIT_IX_ROLLBACK_DATETIME"); ?>:</td>
			<td>
	...
	{
		echo GetMessage("KIT_IX_ROLLBACK_DATES_NOTFOUND");
	}
	?>
<?
$tabControl->EndTab();
...
{
?>
	<input type="button" id="start_button" value="<?echo GetMessage("KIT_IX_ROLLBACK_START_BUTTON")?>" OnClick="StartRollback();" class="adm-btn-save">
	<input type="button" id="stop_button" value="<?=GetMessage("KIT_IX_ROLLBACK_STOP_BUTTON")?>" OnClick="StopRollback();" disabled>
	<input type="button" id="continue_button" value="<?=GetMessage("KIT_IX_ROLLBACK_CONTINUE_BUTTON")?>" OnClick="ContinueRollback();" disabled>
...
<?
echo BeginNote();
echo GetMessage("KIT_IX_ROLLBACK_BOTTOM_NOTE");
echo EndNote();
