				  ...
				  <div class="kit-ix-select-mapping-wrap" data-nc-message="<?echo GetMessage("KIT_IX_NOT_CHOOSE")?>">
					...
					if(array_key_exists($xmlId, $arMap2))
					{
						...
						foreach($arMap2[$xmlId] as $val)
						{
							...
							{
								echo '<div class="kit-ix-select-mapping kit-ix-select-mapping-full" data-xml-id="'.htmlspecialcharsbx($xmlId).'">'.
										...
										'<a id="field_settings_0'.$index.'" href="javascript:void(0)" onclick="ESettings.ShowSelectMappingSettings(event, this)" class="kit-ix-select-mapping-settings'.(strlen($val['EXTRA'])==0 ? ' inactive' : '').'" title="'.GetMessage("KIT_IX_SETTINGS_BTN").'" data-group="offproperty"><input type="hidden" name="MAP[MAP]['.$index.'][EXTRA]" value="'.htmlspecialcharsbx($val['EXTRA']).'"></a>'.
									'</div>';
								$index++;
								$isFields = true;
							}
					...
					if(!$isFields)
					{
						echo '<div class="kit-ix-select-mapping" data-xml-id="'.htmlspecialcharsbx($xmlId).'">'.
								...
								'<a href="javascript:void(0)" onclick="ESettings.ShowSelectMappingSettings(event, this)" class="kit-ix-select-mapping-settings inactive" title="'.GetMessage("KIT_IX_SETTINGS_BTN").'" data-group="offproperty"></a>'.
							'</div>';
					}
					?>
				  </div>
