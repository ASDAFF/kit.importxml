...
{
	$OFFER_IBLOCK_ID = \Bitrix\KitImportxml\Utils::GetOfferIblock($IBLOCK_ID);
...
elseif($_POST['action']=='save_margin_template')
{
	...
	$arMarginTemplates = \Bitrix\KitImportxml\Extrasettings::SaveMarginTemplate($arPost);
}
elseif($_POST['action']=='delete_margin_template')
{
	$arMarginTemplates = \Bitrix\KitImportxml\Extrasettings::DeleteMarginTemplate($_POST['template_id']);
}
...
<form action="" method="post" enctype="multipart/form-data" name="field_settings" class="kit_ix_settings_form">
	...
	<table width="100%">
			...
			<tr>
				...
				<td class="adm-detail-content-cell-r">
					...
					<select name="<?echo $fName;?>">
						...
						<option value="text" <?if($val=='text'){echo 'selected';}?>><?echo GetMessage("KIT_IX_SETTINGS_HTML_TEXT");?></option>
						<option value="html" <?if($val=='html'){echo 'selected';}?>><?echo GetMessage("KIT_IX_SETTINGS_HTML_HTML");?></option>
					</select>
				</td>
			</tr>
		<?}?>
			...
			<tr>
				...
				<td class="adm-detail-content-cell-r">
					...
					<select name="<?echo $fName;?>">
						...
						<option value="LOGIN" <?if($val=='LOGIN'){echo 'selected';}?>><?echo GetMessage("KIT_IX_SETTINGS_USER_UID_LOGIN");?></option>
						<option value="EMAIL" <?if($val=='EMAIL'){echo 'selected';}?>><?echo GetMessage("KIT_IX_SETTINGS_USER_UID_EMAIL");?></option>
						<option value="XML_ID" <?if($val=='XML_ID'){echo 'selected';}?>><?echo GetMessage("KIT_IX_SETTINGS_USER_UID_XML_ID");?></option>
					</select>
				</td>
			</tr>
		<?}?>
		...
		<?if($fieldXpath!=='none' /*!$bOtherField*/){?>
			...
			<tr>
				<td class="kit-ix-settings-margin-container" colspan="2">
					...
					?>
					<a href="javascript:void(0)" onclick="return ESettings.AddConversion(this, event)"><?echo GetMessage("KIT_IX_SETTINGS_CONDITION_ADD_VALUE");?></a>
				</td>
			</tr>
		<?}?>
		...
		<?if($bPrice){
			...
			<tr class="heading">
				<td colspan="2">
					<div class="kit-ix-settings-header-links">
						<div class="kit-ix-settings-header-links-inner">
							<a href="javascript:void(0)" onclick="ESettings.ShowMarginTemplateBlockLoad(this)"><?echo GetMessage("KIT_IX_SETTINGS_LOAD_TEMPLATE"); ?></a> /
							<a href="javascript:void(0)" onclick="ESettings.ShowMarginTemplateBlock(this)"><?echo GetMessage("KIT_IX_SETTINGS_SAVE_TEMPLATE"); ?></a>
						</div>
						<div class="kit-ix-settings-margin-templates" id="margin_templates">
							<div class="kit-ix-settings-margin-templates-inner">
								<?echo GetMessage("KIT_IX_SETTINGS_MARGIN_CHOOSE_EXISTS_TEMPLATE"); ?><br>
								<select name="MARGIN_TEMPLATE_ID">
									<option value=""><?echo GetMessage("KIT_IX_SETTINGS_MARGIN_NOT_CHOOSE"); ?></option>
									<?
								...
								<?echo GetMessage("KIT_IX_SETTINGS_MARGIN_NEW_TEMPLATE"); ?><br>
								<input type="text" name="MARGIN_TEMPLATE_NAME" value="" placeholder="<?echo GetMessage("KIT_IX_SETTINGS_MARGIN_TEMPLATE_NAME"); ?>"><br>
								<input type="submit" onclick="return ESettings.SaveMarginTemplate(this, '<?echo GetMessage("KIT_IX_SETTINGS_TEMPLATE_SAVED"); ?>');" name="save" value="<?echo GetMessage("KIT_IX_SETTINGS_SAVE_BTN"); ?>">
							</div>
						</div>
						<div class="kit-ix-settings-margin-templates" id="margin_templates_load">
							<div class="kit-ix-settings-margin-templates-inner">
								<?echo GetMessage("KIT_IX_SETTINGS_MARGIN_CHOOSE_TEMPLATE"); ?><br>
								<select name="MARGIN_TEMPLATE_ID">
									<option value=""><?echo GetMessage("KIT_IX_SETTINGS_MARGIN_NOT_CHOOSE"); ?></option>
									<?
									foreach($arMarginTemplates as $key=>$template)
								...
								<a href="javascript:void(0)" onclick="ESettings.RemoveMarginTemplate(this, '<?echo GetMessage("KIT_IX_SETTINGS_TEMPLATE_DELETED"); ?>')" title="<?echo GetMessage("KIT_IX_SETTINGS_DELETE"); ?>" class="delete"></a>
								<input type="submit" onclick="return ESettings.LoadMarginTemplate(this);" name="save" value="<?echo GetMessage("KIT_IX_SETTINGS_LOAD_BTN"); ?>">
							</div>
						</div>
					</div>
					<?echo GetMessage("KIT_IX_SETTINGS_MARGIN_TITLE"); ?>
				</td>
			</tr>
			<tr>
				<td colspan="2" class="kit-ix-settings-margin-container">
					<div id="settings_margins">
						<?
						foreach($val as $k=>$v)
						{
						?>
							<div class="kit-ix-settings-margin" style="display: <?=($showMargin ? 'block' : 'none')?>;">
								<?echo GetMessage("KIT_IX_SETTINGS_APPLY"); ?> <select name="<?=$fName?>[TYPE][]"><option value="1" <?=($v['TYPE']==1 ? 'selected' : '')?>><?echo GetMessage("KIT_IX_SETTINGS_APPLY_MARGIN"); ?></option><option value="-1" <?=($v['TYPE']==-1 ? 'selected' : '')?>><?echo GetMessage("KIT_IX_SETTINGS_APPLY_DISCOUNT"); ?></option></select>
								<input type="text" name="<?=$fName?>[PERCENT][]" value="<?=htmlspecialcharsbx($v['PERCENT'])?>">
								<select name="<?=$fName?>[PERCENT_TYPE][]"><option value="P" <?=($v['PERCENT_TYPE']=='P' ? 'selected' : '')?>><?echo GetMessage("KIT_IX_SETTINGS_TYPE_PERCENT"); ?></option><option value="F" <?=($v['PERCENT_TYPE']=='F' ? 'selected' : '')?>><?echo GetMessage("KIT_IX_SETTINGS_TYPE_FIX"); ?></option></select>
								<?echo GetMessage("KIT_IX_SETTINGS_AT_PRICE"); ?> <?echo GetMessage("KIT_IX_SETTINGS_FROM"); ?> <input type="text" name="<?=$fName?>[PRICE_FROM][]" value="<?=htmlspecialcharsbx($v['PRICE_FROM'])?>">
								<?echo GetMessage("KIT_IX_SETTINGS_TO"); ?> <input type="text" name="<?=$fName?>[PRICE_TO][]" value="<?=htmlspecialcharsbx($v['PRICE_TO'])?>">
								<a href="javascript:void(0)" onclick="ESettings.RemoveMargin(this)" title="<?echo GetMessage("KIT_IX_SETTINGS_DELETE"); ?>" class="delete"></a>
							</div>
						<?
						}
						?>
						<input type="button" value="<?echo GetMessage("KIT_IX_SETTINGS_ADD_MARGIN_DISCOUNT"); ?>" onclick="ESettings.AddMargin(this)">
					</div>
				</td>
			</tr>
			...
			<tr class="heading">
				<td colspan="2">
					<?echo GetMessage("KIT_IX_SETTINGS_PRICE_PROCESSING"); ?>
				</td>
			</tr>
			<tr>
				...
				<td class="adm-detail-content-cell-r">
					...
					<select name="<?=$fName?>">
						...
						<option value="ROUND" <?if($val=='ROUND') echo 'selected';?>><?echo GetMessage("KIT_IX_SETTINGS_PRICE_ROUND_ROUND");?></option>
						<option value="CEIL" <?if($val=='CEIL') echo 'selected';?>><?echo GetMessage("KIT_IX_SETTINGS_PRICE_ROUND_CEIL");?></option>
						<option value="FLOOR" <?if($val=='FLOOR') echo 'selected';?>><?echo GetMessage("KIT_IX_SETTINGS_PRICE_ROUND_FLOOR");?></option>
					</select>
				</td>
			</tr>
			<tr>
		...
		<?/*if($bPrice && !empty($arCurrency)){?>
			<tr>
				<td class="adm-detail-content-cell-l"><?echo GetMessage("KIT_IX_FIELD_CURRENCY");?>:</td>
				<td class="adm-detail-content-cell-r">			
			...
			<tr>
				<td class="adm-detail-content-cell-l"><?echo GetMessage("KIT_IX_CONVERT_CURRENCY");?>:</td>
				<td class="adm-detail-content-cell-r">			
					<select name="CONVERT_CURRENCY">
						<option value=""><?echo GetMessage("KIT_IX_CONVERT_NO_CHOOSE");?></option>
					<?
					$lcur = CCurrency::GetList(($by="sort"), ($order1="asc"), LANGUAGE_ID);
					foreach($arCurrency as $item)
					{
			...
			<tr>
				<td class="adm-detail-content-cell-l"><?echo GetMessage("KIT_IX_PRICE_MARGIN");?>:</td>
				<td class="adm-detail-content-cell-r">			
				...
				<td class="adm-detail-content-cell-r">
					...
					foreach($arVals as $k=>$v)
					{
						...
						$select = '<select name="'.$fName2.'" onchange="ESettings.OnValChange(this)">'.
								...
								'<option value="contain" '.($v2=='contain' ? 'selected' : '').'>'.GetMessage("KIT_IX_SETTINGS_FILTER_VAL_CONTAIN").'</option>'.
								'<option value="begin" '.($v2=='begin' ? 'selected' : '').'>'.GetMessage("KIT_IX_SETTINGS_FILTER_VAL_BEGIN").'</option>'.
								'<option value="end" '.($v2=='end' ? 'selected' : '').'>'.GetMessage("KIT_IX_SETTINGS_FILTER_VAL_END").'</option>'.
								'<option value="gt" '.($v2=='gt' ? 'selected' : '').'>'.GetMessage("KIT_IX_SETTINGS_FILTER_VAL_GT").'</option>'.
								'<option value="lt" '.($v2=='lt' ? 'selected' : '').'>'.GetMessage("KIT_IX_SETTINGS_FILTER_VAL_LT").'</option>'.
			...
			<tr>
				...
				<td class="adm-detail-content-cell-r">
					...
					<select name="<?=$fName?>">
						...
						<option value="ADD_BEFORE"<?if($val=='ADD_BEFORE'){echo 'selected';}?>><?echo GetMessage("KIT_IX_SETTINGS_LOADING_MODE_BEFORE");?></option>
						<option value="ADD_AFTER"<?if($val=='ADD_AFTER'){echo 'selected';}?>><?echo GetMessage("KIT_IX_SETTINGS_LOADING_MODE_AFTER");?></option>
					</select>
				</td>
			</tr>
		<?}?>
		...
		if(!$bOtherField || $bProfileUrl)
		{
			...
			<tr>
				<td class="kit-ix-settings-margin-container" colspan="2" id="kit-ix-conv-wrap1">
						...
						echo '<div class="kit-ix-settings-conversion" '.(!$showCondition ? 'style="display: none;"' : '').'>'.
								...
								' <select name="'.$fName.'[THEN][]">'.
									...
									'<optgroup label="'.GetMessage("KIT_IX_SETTINGS_CONVERSION_THEN_GROUP_OTHER").'">'.
										...
										'<option value="NOT_LOAD_ELEMENT" '.($v['THEN']=='NOT_LOAD_ELEMENT' ? 'selected' : '').'>'.GetMessage("KIT_IX_SETTINGS_CONVERSION_THEN_NOT_LOAD_ELEMENT").'</option>'.
										'<option value="EXPRESSION" '.($v['THEN']=='EXPRESSION' ? 'selected' : '').'>'.GetMessage("KIT_IX_SETTINGS_CONVERSION_THEN_EXPRESSION").'</option>'.
									'</optgroup>'.
								'</select> '.
								'<input type="text" name="'.$fName.'[TO][]" value="'.htmlspecialcharsbx($v['TO']).'">'.
					...
					?>
					<a href="javascript:void(0)" onclick="return ESettings.AddConversion(this, event)"><?echo GetMessage("KIT_IX_SETTINGS_CONVERSION_ADD_VALUE");?></a>
				</td>
			</tr>
		<?}?>
	</table>
