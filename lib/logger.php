...
class Logger {
	...
	public function SetExecId(&$execId)
	{
		...
		if($execId < 1 /*&& $this->saveLog*/)
		{
			if($this->removeOldStat)
			{
				...
				{
					$dbRes = \Bitrix\KitImportxml\ProfileExecTable::getList(array('filter'=>array('PROFILE_ID'=>intval($this->profileId)), 'order'=>array('ID'=>'DESC'), 'select'=>array('ID'), 'limit'=>max($this->countLastSaveExecProc, $this->countLastSaveExec)));
				...
				\Bitrix\KitImportxml\ProfileExecTable::deleteByProfile($this->profileId, $arLastProcIds);
				\Bitrix\KitImportxml\ProfileExecStatTable::deleteByProfile($this->profileId, $arLastIds);
			}
			else
			{
				$dbRes = \Bitrix\KitImportxml\ProfileExecTable::getList(array('filter'=>array('PROFILE_ID'=>intval($this->profileId), 'PROFILE_EXEC_STAT.ID'=>false, array('LOGIC'=>'OR', array('!DATE_FINISH'=>false), array('<DATE_START'=>ConvertTimeStamp(time()-7*24*60*60, 'FULL')))), 'order'=>array('ID'=>'DESC'), 'select'=>array('ID'), 'limit'=>999, 'offset'=>$this->countLastSaveExecProc));
				while($arr = $dbRes->Fetch())
				{
					\Bitrix\KitImportxml\ProfileExecTable::delete($arr['ID']);
				}
			}
			
			$dbRes = \Bitrix\KitImportxml\ProfileExecTable::add(array(
	...
	public function FinishExec($arParams)
	{
		...
		\Bitrix\KitImportxml\ProfileExecTable::update($this->execId, array(
	...
	public function SaveElementChanges($ID)
	{
		...
		{
			$dbRes = \Bitrix\KitImportxml\ProfileExecStatTable::add($arFields);
		}
	}
	...
	public function SaveElementChangesMass()
	{
		...
		$entity = new \Bitrix\KitImportxml\ProfileExecStatTable();
	...
	public function SaveSectionChanges($ID)
	{
		...
		$dbRes = \Bitrix\KitImportxml\ProfileExecStatTable::add(array(
			'PROFILE_ID' => $this->profileId,
			'PROFILE_EXEC_ID' => $this->execId,
			'DATE_EXEC' => new \Bitrix\Main\Type\DateTime(),
	...
	public function GetElementDescription($description)
	{
		...
		if(!empty($arFieldsFilter))
		{
			...
			$newDesc .= '<p><b>'.GetMessage("KIT_IX_EVENTRES_GROUP_FILTER").'</b></p><ul>';
		...
		{
			$arFieldNames = $this->fl->GetIblockElementFieldsForStat();
			$newDesc .= '<p><b>'.GetMessage("KIT_IX_EVENTRES_GROUP_FIELDS").'</b></p><ul>';
			foreach($arFieldsElement as $k=>$v)
			{
				if(!isset($arFieldNames[$k]))
				{
					if($k=='IE_IBLOCK_SECTION')
					{
						...
						$newDesc .= '<li><b>'.GetMessage("KIT_IX_EVENTRES_SECTION_ID").':</b> ';
						if(strlen($value) > 0) $newDesc .= htmlentities($value);
						if(strlen($oldvalue) > 0)
						{
							$newDesc .= '<div><b>'.GetMessage("KIT_IX_EVENTRES_OLD_VALUE").'</b> '.htmlentities($oldvalue).'</div>';
						}
						$newDesc .= '</li>';
					}
					continue;
		...
		if(!empty($arFieldsIpropTemp))
		{
			...
			$newDesc .= '<p><b>'.GetMessage("KIT_IX_EVENTRES_GROUP_IPROP_TEMP").'</b></p><ul>';
		...
		{
			$arFieldProps = $this->fl->GetAllIblockProperties();
			$newDesc .= '<p><b>'.GetMessage("KIT_IX_EVENTRES_GROUP_PROPERTIES").'</b></p><ul>';
		...
		if(!empty($arFieldsProduct))
		{
			...
			$newDesc .= '<p><b>'.GetMessage("KIT_IX_EVENTRES_GROUP_CATALOG").'</b></p><ul>';
	...
	public function GetSectionDescription($description, $IBLOCK_ID = false)
	{
		...
		if(!empty($arFieldsSection))
		{
			...
			foreach($arFieldsSection as $k=>$v)
			{
				...
				if(strlen($oldvalue) > 0)
				{
					$newDesc .= '<div><b>'.GetMessage("KIT_IX_EVENTRES_OLD_VALUE").'</b> '.$oldvalue.'</div>';
				}
				$newDesc .= '</li>';
			}
			$newDesc .= '</ul>';
