...
class XMLViewer 
{
	...
	{
		$oProfile = \Bitrix\KitImportxml\Profile::getInstance(strlen($this->suffix)==0 ? 'iblock' : $this->suffix);
				...
				foreach($v as $k2=>$v2)
				{
					...
					if(is_array($v2))
					{
						foreach($v2 as $k3=>$v3)
						{
							if(strlen(trim($v3)) > 0)
							{
								$arTags[$path.'[@'.$k2.'="'.$v3.'"]'] = Loc::getMessage("KIT_IX_VALUE").' '.$path.'[@'.$k2.'="'.$v3.'"]';
							}
						}
					}
				}
	...
	public function GetXpathVals($xpath, $parentXpath='', $arFieldParams=array(), $arProfileParams=array())
	{
		...
		{
			$arExtra = array();
			\Bitrix\KitImportxml\Extrasettings::HandleParams($arExtra, $arFieldParams);
			...
			$ie = new \Bitrix\KitImportxml\Importer(substr($this->filename, strlen($_SERVER['DOCUMENT_ROOT'])), $arProfileParams, array(), array());
	...
	public function CheckDefaultParams(&$sd, &$s, &$se, $arStruct)
	{
		...
		if(count($arPaths) > 0)
		{
			...
			if($sd['IBLOCK_ID'] > 0 && Loader::includeModule('iblock'))
			{
				...
				if($OFFER_IBLOCK_ID = \Bitrix\KitImportxml\Utils::GetOfferIblock($sd['IBLOCK_ID']))
				{
					if(class_exists('\Bitrix\Iblock\PropertyTable'))
					{
