...
class Avsautoru {
	...
	public static function GetDownloadFile($arParams, $maxTime=10)
	{
		...
		$arHeaders = array(
			'User-Agent' => \Bitrix\KitImportxml\Utils::GetUserAgent(),
		);
		...
		if(\Bitrix\KitImportxml\Utils::PathContianApiPages($path))
		{
			$path = \Bitrix\KitImportxml\Utils::PathReplaceApiPages($path);
		}
		...
		if(strlen($handler) > 0)
		{
			...
			if($path)
			{
				$client = \Bitrix\KitImportxml\Utils::GetHttpClient(array('disableSslVerification'=>true), $arHeaders, $arCookies, $path);
				$val = $client->get($path);
			}
			$res = \Bitrix\KitImportxml\Utils::ExecuteFilterExpression($val, $handler, '', $arCookies);
			$path = $res;
		}
