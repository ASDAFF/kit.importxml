...
class Profile {
	...
	public function CheckStorage()
	{
		...
		/*profile_exec*/
		$tEntity = new \Bitrix\KitImportxml\ProfileExecTable();
		...
		/*profile_exec_stat*/
		$tEntity = new \Bitrix\KitImportxml\ProfileExecStatTable();
		$tblName = $tEntity->getTableName();
		$conn = $tEntity->getEntity()->getConnection();
		if(!$conn->isTableExists($tblName))
		{
		...
		/*profile_changes*/
		$tEntity = new \Bitrix\KitImportxml\ProfileChangesTable();
		$tblName = $tEntity->getTableName();
		$conn = $tEntity->getEntity()->getConnection();
		if(!$conn->isTableExists($tblName))
		{
			...
			if(is_callable(array($conn, 'isIndexExists')) && !$conn->isIndexExists($tblName, array('IBLOCK_ID', 'NAME')))
			{
				...
				/*else
				{
					\CAdminNotify::add(array(
						"MESSAGE" => Loc::getMessage("KIT_IX_CREATE_ELEMENT_NAME_INDEX", array(
	...
	private function GetImportEntity()
	{
		if(!$this->importEntity)
		{
			if($this->suffix=='highload')
			{
				$this->importEntity = new \Bitrix\KitImportxml\ProfileElementHlTable();
			}
			else
			{
				$this->importEntity = new \Bitrix\KitImportxml\ProfileElementTable();
			}
		}
	...
	public function Add($name, $fid = false)
	{
		...
		else
		{
			...
			if($fid!==false)
			{
				\CFile::UpdateExternalId($fid, 'kit_importxml_'.($this->suffix=='highload' ? 'hl' : '').$ID);
			}
			return $ID;
		}
	}
	...
	{
		return '\Bitrix\KitImportxml\ProfileChangesTable';
	}
	...
	public function Delete($ID)
	{
		...
		\Bitrix\KitImportxml\Utils::DeleteFilesByExtId('kit_importxml_'.($this->suffix=='highload' ? 'hl' : '').$ID);
		\Bitrix\KitImportxml\ProfileExecTable::deleteByProfile($ID+1);
		\Bitrix\KitImportxml\ProfileExecStatTable::deleteByProfile($ID+1);
	}
	...
	public static function EncodeProfileParams($arParams)
	{
		...
		$arParams['ENCODING'] = \Bitrix\KitImportxml\Utils::getSiteEncoding();
	...
	public static function DecodeProfileParams($paramStr, $checkEncoding=true)
	{
		...
		if(!is_array($arParams)) $arParams = array();
		if($checkEncoding && $arParams['ENCODING'] && $arParams['ENCODING'] != \Bitrix\KitImportxml\Utils::getSiteEncoding())
		{
			$arParams = \Bitrix\Main\Text\Encoding::convertEncodingArray($arParams, $arParams['ENCODING'], \Bitrix\KitImportxml\Utils::getSiteEncoding());
		}
		return $arParams;
	}
	...
	public function OnEndImport($file, $arParams, $arErrors=array(), $arProfParams=array())
	{
				...
				|| (\Bitrix\Main\Config\Option::get(static::$moduleId, 'NOTIFY_MODE', 'NONE')=='CRON' && $this->importMode=='CRON')))
		{
			...
			foreach(array('TOTAL_LINE', 'CORRECT_LINE', 'ERROR_LINE', 'ELEMENT_ADDED_LINE', 'ELEMENT_UPDATED_LINE', 'ELEMENT_CHANGED_LINE', 'ELEMENT_REMOVED_LINE', 'KILLED_LINE', 'ZERO_STOCK_LINE', 'OLD_REMOVED_LINE', 'SKU_ADDED_LINE', 'SKU_UPDATED_LINE', 'SKU_CHANGED_LINE', 'OFFER_KILLED_LINE', 'OFFER_ZERO_STOCK_LINE', 'OFFER_OLD_REMOVED_LINE', 'SECTION_ADDED_LINE', 'SECTION_UPDATED_LINE', 'SECTION_DEACTIVATE_LINE', 'SECTION_REMOVE_LINE', 'ERRORS') as $k=>$v)
			{
				...
				{
					$arEventData['STAT_BLOCK'] .= ($v=='ERRORS' ? "\r\n\r\n" : '').Loc::getMessage("KIT_IX_EVENT_".$v).": ".($v=='ERRORS' ? "\r\n" : '').$arEventData[$v]."\r\n";
				}
			}
				if(array_key_exists('STAT_LINK', $arEventData))
				{
					$arEventData['STAT_BLOCK'] .= "\r\n".Loc::getMessage("KIT_IX_EVENT_STAT_LINK").$arEventData['STAT_LINK'];
				}
	...
	public function OnBreakImport($reason='')
	{
		...
		if($this->suffix!='highload')
		{
			$reason = (strlen(Loc::getMessage("KIT_IX_BREAK_REASON_".ToUpper($reason))) > 0 ? Loc::getMessage("KIT_IX_BREAK_REASON_".ToUpper($reason)) : $reason);
					...
					|| (\Bitrix\Main\Config\Option::get(static::$moduleId, 'NOTIFY_MODE', 'NONE')=='CRON' && $this->importMode=='CRON')))
			{
				...
				{
					\CEvent::Send('KIT_IMPORT_BREAK', $this->GetDefaultSiteId(), $arEventData, 'Y', '', array($arProfile['DATA_FILE_ID']));
				}
				else
				{
					\CEvent::Send('KIT_IMPORT_BREAK', $this->GetDefaultSiteId(), $arEventData);
				}
			}
		}
	...
	public function OnFileNotChanged()
	{
		if($this->suffix!='highload')
		{
					...
					|| (\Bitrix\Main\Config\Option::get(static::$moduleId, 'NOTIFY_MODE', 'NONE')=='CRON' && $this->importMode=='CRON')))
			{
				...
				if(\Bitrix\Main\Config\Option::get(static::$moduleId, 'NOTIFY_WITH_FILE', 'N')=='Y')
				{
					\CEvent::Send('KIT_IMPORT_FILE_NOT_CHANGED', $this->GetDefaultSiteId(), $arEventData, 'Y', '', array($arProfile['DATA_FILE_ID']));
				}
				else
				{
					\CEvent::Send('KIT_IMPORT_FILE_NOT_CHANGED', $this->GetDefaultSiteId(), $arEventData);
				}
			}
		}
		return $arEventData;
	...
	public function CheckEventOnBreakImport()
	{
		...
		if(!$dbRes->Fetch())
		{
			...
			$et->Add(array(
				"LID"           => "ru",
				"EVENT_NAME"    => $eventName,
				"NAME"          => Loc::getMessage("KIT_IX_EVENT_IMPORT_BREAK"),
				"DESCRIPTION"   => 
					"#PROFILE_NAME# - ".Loc::getMessage("KIT_IX_EVENT_PROFILE_NAME")."\r\n".
					"#IMPORT_START_DATETIME# - ".Loc::getMessage("KIT_IX_EVENT_TIME_BEGIN")."\r\n".
					"#IMPORT_BREAK_REASON# - ".Loc::getMessage("KIT_IX_EVENT_IMPORT_BREAK_REASON")
				));
		}
		$dbRes = \CEventMessage::GetList(($by='id'), ($order='desc'), array('TYPE_ID'=>$eventName));
		if(!$dbRes->Fetch())
		{
			...
			$emess->Add(array(
				...
				'EMAIL_FROM' => '#DEFAULT_EMAIL_FROM#',
				'EMAIL_TO' => '#EMAIL_TO#',
				'SUBJECT' => '#SITE_NAME#: '.Loc::getMessage("KIT_IX_EVENT_BREAK_PROFILE").' "#PROFILE_NAME#"',
				'BODY_TYPE' => 'html',
				'MESSAGE' => 
					Loc::getMessage("KIT_IX_EVENT_PROFILE_NAME").": #PROFILE_NAME#<br>\r\n".
					Loc::getMessage("KIT_IX_EVENT_TIME_BEGIN").": #IMPORT_START_DATETIME#<br>\r\n".
					Loc::getMessage("KIT_IX_EVENT_IMPORT_BREAK_REASON").": #IMPORT_BREAK_REASON#"
			));
		}
	}
	...
	{
		$eventName = 'KIT_IMPORT_FILE_NOT_CHANGED';
		$dbRes = \CEventType::GetList(array('TYPE_ID'=>$eventName));
		if(!$dbRes->Fetch())
		{
			$et = new \CEventType();
			$et->Add(array(
				"LID"           => "ru",
				"EVENT_NAME"    => $eventName,
				"NAME"          => Loc::getMessage("KIT_IX_EVENT_IMPORT_FILE_NOT_CHANGED"),
				"DESCRIPTION"   => 
					"#PROFILE_NAME# - ".Loc::getMessage("KIT_IX_EVENT_PROFILE_NAME")."\r\n".
					"#IMPORT_START_DATETIME# - ".Loc::getMessage("KIT_IX_EVENT_TIME_BEGIN")
				));
		...
		if(!$dbRes->Fetch())
		{
			...
			$emess->Add(array(
				...
				'EMAIL_FROM' => '#DEFAULT_EMAIL_FROM#',
				'EMAIL_TO' => '#EMAIL_TO#',
				'SUBJECT' => '#SITE_NAME#: '.Loc::getMessage("KIT_IX_EVENT_FILE_NOT_CHANGED_PROFILE"),
				'BODY_TYPE' => 'html',
				'MESSAGE' => 
					Loc::getMessage("KIT_IX_EVENT_PROFILE_NAME").": #PROFILE_NAME#<br>\r\n".
					Loc::getMessage("KIT_IX_EVENT_TIME_BEGIN").": #IMPORT_START_DATETIME#"
