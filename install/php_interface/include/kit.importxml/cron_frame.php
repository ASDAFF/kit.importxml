...
foreach($arProfiles as $PROFILE_ID)
{
	...
	if($params['EXT_DATA_FILE'] || $params['EMAIL_DATA_FILE'])
	{
		...
		{
			if($newFileId = \Bitrix\KitImportxml\SMail::GetNewFile($SETTINGS_DEFAULT['EMAIL_DATA_FILE'], 86400, 'kit_importxml_'.$PROFILE_ID))
			{
		...
		else
		{
			if(!$newFileId && $arFile)
			{
				...
				$arFile['del_old'] = 'Y';
				if($newFileId = \Bitrix\KitImportxml\Utils::SaveFile($arFile, $moduleId))
				{
	...
	if(!file_exists($_SERVER["DOCUMENT_ROOT"].$DATA_FILE_NAME))
	{
		...
		else
		{
			...
			$ie = new \Bitrix\KitImportxml\Importer($DATA_FILE_NAME, $params, $EXTRASETTINGS, array_merge($arParams, array('NOT_CHANGE_PROFILE'=>'Y')), $pid);
