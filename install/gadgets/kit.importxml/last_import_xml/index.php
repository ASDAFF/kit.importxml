...
if(!empty($arProfiles))
{
	echo '<table border="1">'.
		'<tr>'.
			'<th>'.Loc::getMessage('GD_KIT_IX_PROFILE_ID').'</th>'.
			'<th>'.Loc::getMessage('GD_KIT_IX_PROFILE_NAME').'</th>'.
			'<th>'.Loc::getMessage('GD_KIT_IX_PROFILE_DATE_START').'</th>'.
			'<th>'.Loc::getMessage('GD_KIT_IX_PROFILE_DATE_FINISH').'</th>'.
			'<th>'.Loc::getMessage('GD_KIT_IX_PROFILE_STATUS').'</th>'.
			'<th></th>'.
		'</tr>';
		...
		echo '<tr'.($arStatus['STATUS']=='ERROR' ? ' style="background: #ffdddd;"' : '').'>'.
				...
				'<td>'.
					'<form method="post" action="/bitrix/admin/'.$moduleFilePrefix.'.php?lang='.LANGUAGE_ID.'" target="_blank">'.
						...
						'<input type="submit" value="'.Loc::getMessage('GD_KIT_IX_RUN_IMPORT').'">'.
					'</form>'.
				'</td>'.
			'</tr>';
	}
...
{
	echo Loc::getMessage('GD_KIT_IX_NO_DATA');
}
?>
